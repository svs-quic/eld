...
## Discarding assembler temporary symbols

Use `--discard-locals` (or its short form `-X`) to remove local symbols
whose names begin with ``.L`` from the output symbol table. These symbols are
commonly generated by assemblers for temporary labels and are generally not
useful to consumers of the linked image.

Ordinary local symbols are preserved. For example, given an object containing
both `.Ltemporary` and `local_helper`,

```
$ ld.eld --discard-locals input.o -o output.elf
```

removes `.Ltemporary` while retaining `local_helper`. Use
`--discard-all` (or `-x`) when all local symbols should be removed.

The filtering also applies when relocation sections are emitted. On RISC-V,
`--keep-labels` does not restore `.L` temporaries removed by
`--discard-locals`.
